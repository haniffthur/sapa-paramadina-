...
<div class="flex justify-between items-center mb-6">
    ...
    <form action="{{ route('admin.users.index') }}" method="GET" class="flex gap-2">
        ...
        <button type="submit" class="bg-blue-900 text-white px-4 py-2 rounded-xl text-sm">
            ...
            <tr class="hover:bg-gray-50 transition">
                <td class="p-4">
                    <div class="flex items-center gap-3">
                        <img src="{{ $user->avatar }}" class="w-10 h-10 rounded-full border shadow-sm">
                        <div>
                ...
                <td class="p-4 text-right">
                    <form action="{{ route('admin.users.update-role', $user->id) }}" method="POST" class="flex justify-end gap-2">
                        @csrf
                        <select name="role" class="text-xs border rounded-lg px-2 py-1 outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="mahasiswa" {{ $user->role == 'mahasiswa' ? 'selected' : '' }}>Mahasiswa</option>
                            <option value="admin" {{ $user->role == 'admin' ? 'selected' : '' }}>Admin</option>
                            <option value="superadmin" {{ $user->role == 'superadmin' ? 'selected' : '' }}>Superadmin</option>
                        </select>
                        <button type="submit" class="bg-gray-800 text-white p-1.5 rounded-lg hover:bg-black transition">
